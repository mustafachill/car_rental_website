...
// Security Check: Ensure user is a logged-in customer and an ID is provided
if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'customer' || !isset($_GET['id'])) {
    redirect('login.php');
}
...
// --- Fetch Car Details ---
try {
    // Only fetch the non-sensitive fields
    $sql = "SELECT make, model, license_plate, status, mileage, image_path FROM Cars WHERE car_id = ?";
...
try {
    $sql = "SELECT service_date, service_type, notes FROM Maintenance WHERE car_id = ? ORDER BY service_date DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$carId]);
...
$currentTable = 'Cars'; // Keep the Cars tab highlighted for navigation context
include 'header.php';
?>
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($maintenanceHistory)): ?>
                    <tr><td colspan="3">No maintenance history found.</td></tr>
                <?php else: ?>
